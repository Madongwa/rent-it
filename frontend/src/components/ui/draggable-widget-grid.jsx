...
/* ------------------------------------------------------------------ *
 * Types (documented in comments only - this is a plain JS port)
 *
 * WidgetSize: 'sm' | 'wide' | 'tall' | 'lg' - column x row span:
 *   sm 1x1, wide 2x1, tall 1x2, lg 2x2.
 *
 * WidgetItem: { id: string, size: WidgetSize, label?: string }
 *
 * DraggableWidgetGridProps:
 *   items?: WidgetItem[] - initial arrangement. The grid manages the order
 *     after mount.
 *   onChange?: (items) => void - called with the new order after a drop or
 *     a keyboard move.
 *   renderItem?: (item, size) => ReactNode - renders a widget's content.
 *   editable?: boolean - enables dragging and keyboard moves.
 *   maxColumns?: number - upper bound for the number of columns.
 *   cellSize?: number - target cell width in px; the column count is
 *     derived from it.
 *   gap?: number - gap between widgets in px.
 *   radius?: number - corner radius of a widget in px.
 *   className?: string
 * ------------------------------------------------------------------ */
        ...
        <motion.div
            ...
            <motion.div
                ...
                className={`relative isolate flex h-full w-full flex-col overflow-hidden rounded-[var(--widget-radius)] bg-card text-card-foreground ring-inset transition-shadow duration-300 [clip-path:inset(0_round_var(--widget-radius))] ${
                    landed ? 'ring-2 ring-foreground/40' : 'ring-1 ring-border/15'
                }`}>
...
export function DraggableWidgetGrid({
    ...
    useIsoLayoutEffect(() => {
        ...
        const el = grid.current?.querySelector(`[data-widget-id="${CSS.escape(id)}"]`)
        el?.focus()
    }, [items])
